...
function renderVlanTab() {
  ...
  <div class="table-wrap"><table>
    <thead><tr><th>Tag</th><th>Nome</th><th>Sub-rede</th><th>Gateway</th><th>Ações</th></tr></thead>
    <tbody>${vlans.map(v => `<tr><td><span class="badge badge-accent">${esc(v.tag)}</span></td><td class="td-name">${esc(v.nome)}</td><td>${esc(v.subrede || "—")}</td><td>${esc(v.gateway || "—")}</td>
    <td class="td-actions"><button class="btn btn-sm btn-ghost" data-action="edit-vlan" data-id="${esc(v.id)}">Editar</button><button class="btn btn-sm btn-danger" data-action="del-vlan" data-id="${esc(v.id)}">Remover</button></td></tr>`).join("")}</tbody>
  </table></div>`;
}
...
function openVlanForm(existing) {
    ...
    <div class="form-group"><label class="form-label">Sub-rede</label><input class="form-input" id="f_vlsub" value="${esc(v.subrede)}" placeholder="192.168.10.0/24"/></div>
